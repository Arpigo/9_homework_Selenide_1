...
public class Snippets {
    ...
    void selectors_examples() {
        ...
        $("").parent();
        $("").sibling(1); //ищет сверхи вниз, братьев и сестер
        $("").preceding(1); //тоже самое но в обратную сторону
        $("").closest("div");
        $("").ancestor("div"); // the same as closest
        $("div:last-child"); //поиск последнего элемента
    ...
    void actions_examples() {
        ...
        $("").setValue("text"); //полностью очищает поле и вводит текст
        $("").append("text"); //добавляет к уже сущестующему тексту текст
        $("").clear();
        $("").setValue(""); // clear
    ...
    void assertions_examples() {
        ...
        //longer timeouts
        $("").shouldBe(visible, Duration.ofSeconds(30)); //установка таймаута
    ...
    void conditions_examples() {
        ...
        $("").shouldHave(cssValue("font-size", "12")); //проверка цвета или физических элементов

        $("").shouldHave(value("25")); //проверка текста который введен в строку, перед поиском
    ...
    void collections_examples() {
        ...
        $$("div").findBy(text("123")).click(); //  finds first элемент
        ...
        $$("").shouldHave(exactTexts("Alfa", "Beta", "Gamma")); //проверка полного соотвествия текста

        $$("").shouldHave(textsInAnyOrder("Beta", "Gamma", "Alfa")); //проверка порядка элементов в тексте
        $$("").shouldHave(exactTextsCaseSensitiveInAnyOrder("Beta", "Gamma", "Alfa"));

        $$("").shouldHave(itemWithText("Gamma")); // only one text, если будет один элемент
    ...
    void javascript_examples() {
        ...
        long fortytwo = executeJavaScript("return arguments[0]*arguments[1];", 6, 7); //запускает джава скрипт с аргументами и возвращает элемент
